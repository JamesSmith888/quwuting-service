...
会被 Skill 扫描器一并收录 ⇒ **实际加载到的是备份里的旧稿**（实测命中
...
4. **双位置同步约定**（同 quwuting-venue-daily-sync）：
   - 项目路径（Git，源文件）：`quwuting-service/quwuting-announcement-publish/`（仓内相对路径）
...
标题按内容主体选（**09-14/09-17 用户纠正，标题须表达内容主体、着重强调新增门店**，勿一律套「舞讯更新」）：
主为【新增门店】→ `{M}月{D}日新增门店`（**可带数量强调，如「9月17日新增18家门店」**）；含【恢复营业】或「确认营业名单」版式 → `{M}月{D}日舞讯更新`（category=DATA_UPDATE，pinned=true）。
**🚨 09-17 拍板「新增门店公告与每日舞讯公告必须分开」**：两类内容禁合并为一条——舞讯公告不混【新增门店】段，新增门店独立 create+publish 新条（同日可并存多条 DATA_UPDATE）；「更新非重发」仅限同批次信息补全（实证：18 家新增段误并 #51 → 拆分为 #51 还原 + #52 新发）。
