...
- **一期不支持城市筛选**：`city` 随条目返回，仅作消息尾行的城市标签（列表内唯一的位置语义）。
...
| 页面 | 说明 |
|---|---|
| `pages/bulletins` | **信息流**（tab 页）。一条快讯 = 一条**全宽白带**（左右撑满屏幕）+ 底部 16rpx 灰缝，带内按**消息**排版：正文（markdown）→ 尾部（城市 · 时间）→ 表态行；触底分页；尾部免责声明（列表侧无「详情」入口，见 §7.4） |
| `pages/bulletin-detail` | 分享落地。towxml 渲染 markdown + `normalizeAnnouncementLinks` 归一化 `venue://`；含同一套表态行 |
...
- 消息尾部 = `.bul-meta`（城市左、时间右 `margin-left: auto`）；
...
| 3 | **整表重渲染**（收敛一次就重传全文、重挂 towxml 节点树） | 收敛前先算**指纹**（`feedFingerprint` = 日期 + 主题 + 每条 正文/标题/城市/生效时间/表态摘要），与本地渲染结果一致 ⇒ **一个字节都不 setData** | 相同输入不得产生渲染。指纹必须覆盖**所有会影响渲染的输入**：跨零点后相对时间会变、主题决定 towxml 解析产物 —— 两者都必须进指纹，否则渲染出与输入不符的视图 |
...
- [ ] 消息式尾部：城市左 / 时间右；长文案不挤压；无城市时时间仍靠右
