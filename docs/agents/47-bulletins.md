...
> 见 §6.2）。
>
> **2026-09-19 七次迭代：表态字典全量放开（32 → 100）**——用户第三次反馈"快讯表情太少，
> 至少要达到门店列表 item 的表情数量"。后端 `BulletinReactionCode` 的 code 列表改为
> **`EmojiCatalog.allCodes()` 全量派生**（手工列表删除 ⇒ 与前端的手工同步点消失），
> 测试改为"与目录同集同序"断言；前端菜单展开态 100 枚改限高滚动。见 §6.2。
>
> **2026-09-12 五次迭代（纯前端交互）**：表态行由"纯展示"回归**快捷入口**（点已有表情直接
...
### 6.2 字典：共享 emoji 目录**全量 100 枚**（**不是门店字典的子集**；2026-09-11 两次扩容 10 → 16 → 32，2026-09-19 全量放开 32 → 100）

**集合 = `EmojiCatalog` 全量**（声明序 = 展示序：表情与情感 → 人物 → 手势 → 爱心 →
庆祝/符号 → 花卉与酒水），不再手工挑选。
...
- **实现形态 = 域适配器**：后端 `BulletinReactionCode` 引用共享目录 `EmojiCatalog`
  （`emojiOf/labelOf` 直接取目录值），**不复制条目、不自造 emoji**
  （同舞伴域 DancerTagCode 先例）；前端 `constants/bulletin-reactions.ts` 从
  `EMOJI_CATALOG` 派生条目。
- **2026-09-19 全量放开（用户驱动，32 → 100）**：
  - **触发**：用户第三次反馈"快讯的表情太少，至少要达到门店列表 item 的表情数量"。
    门店列表 Picker 展开后 109 格（legacy 17 业务信号 + 目录去重 92），快讯只有 32 格。
  - **根因**：旧理由"信息流要一瞥可辨（TG 频道 reaction 也是小集合）"混淆了两件事——
    **一瞥可辨是"默认可见子集"要解决的问题，不是"砍掉可选集合"的理由**：TG 频道用
    小集合是因为它根本没有展开入口，快讯菜单早有「折叠 8 枚 / 展开全量」两级形态。
    逐次累加（10 → 16 → 32）也暴露手工挑选的固有缺陷：**每次只补"想到"的那几个，
    用户期望是"和门店一样多"**。
  - **口径**：快讯域无 legacy 业务信号，本域上限即目录全集 = 100 项，与门店 109 同量级
    （差的 9 枚全是门店专属业务信号，不属情感表达层、不补齐）。
  - **同步点消失**：后端 `EmojiCatalog.allCodes()`、前端 `EMOJI_CATALOG.map(code)`，
    两端同为目录派生，目录增删自动跟随，零漂移。
  - **前端展示配套**：菜单折叠态 = 高频 8 枚显式集合（`MENU_QUICK_CODES`，"取字典前 8"
    的隐含耦合随目录全量化失效）、展开态限高滚动；`reaction-picker` bulletin 变体
    走面板内 scroll-view 滚动（无「展开全部」入口）。
- **单调性护栏**：零依赖测试 `BulletinReactionCodeTest` 断言"每个 code 必须存在于
  `EmojiCatalog` 且 emoji/label 非空 + 无重复 + **与目录同集同序**"——目录项被删时
  两端同步收敛，不会下发 `emoji: null` 让前端渲染空白格。
...
- **选择**：复用 `components/reaction-picker`，新增 `variant="bulletin"`——
  ...
  WXML 零分支。快讯域无展开（2026-09-19 起面板内 scroll-view 直接承载全量 100 枚）、
  不显示说明页入口（说明页展示的是门店字典）。
...
**2026-09-12 六稿（纯前端交互，服务端零改动）**：用户实测两条 ⇒ ① 表态行从"纯展示"
回归**快捷入口**——点已有表情直接 toggle（与门店列表卡片同源，一人一票迁移规则仍走
`planBulletinReactionToggle` 单点收口）；② 消息操作菜单的呼出范围由"气泡"扩到**整行**
（气泡外的行背景也算）。判据 = **捕获是全局穿透、冒泡才受子节点约束**：想让某个子区域
有独立行为，就不要把监听挂到它的祖先捕获上。顺带把共享组件 `reaction-chips` 的
`zonetap` detail 由 `{}` 补为 `{touchX, touchY}`（同 `chiplongpress` 契约，宿主用它锚定
弹层；门店/舞伴宿主不读，向后兼容）。接口、DTO、表结构、迁移**均无改动**。
前端完整实现与行为矩阵见 `quwuting/docs/agents/47-bulletins.md` §7.4.4。
...
**七次迭代（2026-09-19，表态字典全量放开 32 → 100）**：

- [x] 后端 `BulletinReactionCode`：`CODES` 改为 `EmojiCatalog.allCodes()` 全量派生；
      `BulletinReactionCodeTest` 改为"与目录同集同序"断言
- [x] 前端 `constants/bulletin-reactions.ts`（+ `.js` 镜像）改为 `EMOJI_CATALOG.map(code)` 全量派生
- [ ] 真机验证：菜单展开后 100 枚可滚动、滚动不误触表态
...
- [ ] Picker `variant="bulletin"`：**全量 100 枚滚动网格**、无「展开全部」、无「表情说明」入口
- [ ] 长按 chip 说明弹层锚定位置
