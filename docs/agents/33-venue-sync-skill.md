...
## 表格化差异清单（Step 4 执行规范，2026-09-01 优化；2026-09-10 增表⑤；2026-09-15 高置信全自动）

比对结果按「写库动作」分类为**五张**对比表格。执行分级（2026-09-15 现行口径）：
**表① 高置信 = 永远自动写库 + 自动发公告（不询问、单源日也不降级）**；表②/③/⑤ 需用户逐条放行
（表③ 新店**任何情况都不自动建档**，红线 4）。

| # | 分类 | 内容 | 执行 |
|---|---|---|---|
| ① | 可直接更新 · 开门 | 全源点名（M==S）+ EXACT/ALIAS + 平台 CEASED/SUSPENDED | 🔴 **永远自动**反转 + **自动发公告**，不询问（09-15 用户拍板：「高置信的永远自动发送公告」） |
| ② | 需用户核实 | 源间冲突（0<\|M\|<\|S\|）/ 单源覆盖（\|S\|<2）/ CONTAINED 低置信 | 列「管理员手动核实」清单，逐条放行 |
| ③ | 平台未维护（新店候选） | 全源点名 + UNMATCHED + keyword 交叉验证无命中 | 🔴 **禁止自动建档（09-11 红线 4）**：一律只列「待放行清单」，用户放行后才 batch-create——**即使全源一致 + 无同名也不自动建** |
