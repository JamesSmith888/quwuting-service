...
recommended（默认，复合评分）score = 受守卫的运营权重 sortWeight
      + 行为热度（浏览贡献 = 近30天去重浏览人数×0.3 + 近7天去重浏览人数×0.4
        + ln(1+近30天匿名浏览行数)×1
        —— <b>2026-09-19 二重构</b>，取代 2026-08-27 的「来源加权人次 + ln 压缩」：
        旧口径实测封顶仅 6.4 分（低于 1 次收藏的 8 分），等于让最稀疏的信源主导排序。
        论证与真库实证见 05-venue-heat.md「浏览贡献二重构」）
        + 近30天收藏人数×8（2026-09-01 收敛：收藏总数为累计展示、不计入公式——旧双列
          「收藏总数×10 + 新增×15」是集合包含关系，一次收藏重复计 25 分，且存量永续制造马太；
          2026-09-07 数值校准 15→8；2026-09-19 改人数口径）
        + 近30天新增动态×5（2026-09-01 由动态总数改窗口，admin 内容存量不参与）
        + 近30天评分人数×8 + 近30天正向反馈人数×3（均 2026-09-19 改人数口径）
...
> ⚠️ **本节为 2026-08-27 的历史口径，2026-09-19 已被「人数口径」整体取代，保留作演进记录。**
> 现行口径见 05-venue-heat.md「浏览贡献二重构」。取代原因（真库实证）：旧口径把"有多少人
> 来看"与"被点开多少次"混在一起压，实测封顶仅 6.4 分——低于 1 次收藏（8 分），
> 332 家「有人看、没人收藏」的门店在排序中完全无法体现。
...
**热门门槛联动（2026-08-27 语义收紧；2026-09-19 随人数口径复校准）**：`venue.hot.min-heat-score`（默认 70）作用于行为热度。2026-09-19 真库实测：浏览项由封顶 6.4 升至 ≈59，热门集合 **4 → 5 家**——「大量浏览 + 少量主动信号」重新成为达标路径之一（如 100 UV 30 分 + 5 收藏 40 分），但匿名项 ln 封顶 ≈6 分，刷量仍买不到热门。门槛值保持 70 不动，调整见 `VenueHotProperties` javadoc。

**实施落点（三处镜像 + 唯一事实源）**：
- `VenueHeatWeights`：`VIEW_UV_30D` / `VIEW_UV_7D` / `VIEW_ANON_LN`（唯一事实源，2026-09-19 起；
  2026-08-27 的 `VIEW_SOURCE_*` / `VIEW_RECENCY_7D_MULTIPLIER` 已随口径替换移除）；
- `VenueRepository.VIEW_BEHAVIOR`（JPQL 排序，独立常量，三个相关子查询 + `LN` 为 Hibernate
  注册函数、近7天减法带 `day` 后缀）+ `findHotVenueIds`（native 同构）+ `countHeatCounters`
  2026-09-19 起下发 `viewuv30d` / `viewuv7d` / `viewanon30d`（取代 `weightedviews30d`）；
- `VenueHeatService.computeHeat`（Java，人数 × 权重 + `Math.log1p(匿名行数)`）——viewCount30d（原始 PV）仍下发仅供展示，不再线性进入公式。

**窗口统一（2026-09-08）**：排序/热门口径与热度页实时口径对齐——SQL 镜像 `[今天-30, 今天+1)`（`CURRENT_DATE + 1 day` 上界，DATE 列 `<= CURRENT_DATE`），含今日已发生数据（当日决策场景："今天火的店今天靠前"；生产实证丽莎歌舞厅当日 3 收藏即时反超案例）。2026-08-13 的「排序截至昨日 / 热度页实时」分家口径废止；满意度偏移仍只在热度页（排序看行为热度的分层不变）。
