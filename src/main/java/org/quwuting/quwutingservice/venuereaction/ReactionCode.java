 ...
 * ② label 去判断化：QUIET「人气冷清」→「人气偏少」（"冷清"是情绪判断，"偏少"
 *    是程度事实）、SERVICE_ISSUE「服务问题」→「服务欠佳」（"问题"偏定性）。
 *    **PRICE_HIKE 语义纠正（用户澄清 2026-08-13）**：✌ 剪刀手 ≠ 临时加价，是
 *    **价格差异**（同场舞伴默契收 10 元、个别舞伴收 20 元）——label「舞伴临时
 *    加价」→「收费偏高」（静态价格事实，无"中途变卦"的错误暗示与冤枉）。
 * ③ description（前端展示文案）全量重写为"事实陈述式"：负面描述一律用"遇到过/
 *    发现"句式（个人经历陈述，平台不裁决），不用"有舞伴会/这家店就是"句式
 *    （平台定性）；不点名个体；价格数字仅限"同场对比"语境（如剪刀手 10/20 元），
 *    禁脱离语境的绝对价格——见前端 constants/reactions.ts 注释。
...
public enum ReactionCode {
     ...
     * 是负面标签而非性价比。处理三原则：① 极性 POSITIVE → NEGATIVE（退出热度
     * 公式、进负面信号单独计数），历史数据经 V15 migration 重映射（VALUE → PRICE_HIKE）。
     * <p>
     * 2026-08-13 语义纠正（用户澄清）：✌ 剪刀手 ≠ "中途临时加价"——是**价格差异**
     * 而非**行为变卦**：同场舞伴默契都收 10 元，个别舞伴收 20 元（收费高于同场行情）。
     * 旧 label「舞伴临时加价」与旧描述"中途临时加价、和开场说的价格不一样"均为误读，
     * 且"临时加价"暗示舞伴中途失信，比真实情况更负面、更冤枉人（见前端字典注释）。
     * 纠正后：label「收费偏高」（静态价格事实，无变卦暗示）、description（前端）：
     * 「同场舞伴大多收 10 元，个别要收 20 元」——"同场对比"限定防误伤正常 20 元场、
     * "大多/个别"句式不点名个体、emoji 保留 ✌ 作圈内暗号、不明示"剪刀手"字样。
     */
    PRICE_HIKE("✌", "收费偏高", Polarity.NEGATIVE),
    /**
