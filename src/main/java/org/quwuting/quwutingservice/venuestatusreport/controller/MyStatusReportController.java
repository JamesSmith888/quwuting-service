...
public class MyStatusReportController {
    ...
    /**
     * 当前用户的全部状态上报记录（需登录）。
     * GET /status-reports/mine
     * <p>
     * 返回未撤销记录（含已过期），按报告时间倒序；{@code active} / {@code expiresAt}
     * 由后端 TTL 常量计算，前端不持有 TTL 常量。供详情页「我的上报记录」弹窗使用。
     */
    @GetMapping("/mine")
    public ApiResponse<List<MyStatusReportResponse>> listMyReports() {
        return ApiResponse.ok(statusReportService.listMyReports());
    }
}
