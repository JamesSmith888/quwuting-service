 ...
 * <p>
 * 默认 70 的语义（<b>2026-08-27 浏览贡献重构后校准；2026-09-07 收藏权重 15→8 复校准</b>）：
 * 行为热度的浏览项已由线性 PV 计数改为「来源加权 + 近7天×2 + ln(1+x) 压缩」（见
 * VenueHeatWeights 浏览贡献注释），纯浏览不再能单靠浏览量达标——原"≈ 70 次浏览"的达标
 * 路径关闭，这是预期效果（热门 = 真实人气，光有人看≠热门）。收藏权重 ×8 校准后，
 * 70 ≈ 近30天 9 次收藏、或 6 次收藏 + 2 次评分 + 2 条正向反馈、或 8 次收藏 + 1 次评分
 * 等<b>主动信号组合</b>（纯浏览因 ln 压缩仍无法单列达标）。
...
public record VenueHotProperties(int minHeatScore) {
    ...
    /** 配置缺失/非法时的安全回退：70 分（≈ 近30天 9 次收藏 ×8，或主动信号组合——纯浏览因 ln 压缩不再能单独达标） */
