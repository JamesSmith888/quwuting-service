...
public record VenueHeatResponse(
        ...
        long viewUv30d,
        /**
         * 近30天「跨天复访」的登录用户数（2026-09-15 新增）：同一登录用户在 ≥2 个不同
         * view_date 上到访过本场所的人数——回答"有多少人是真的回头再来"。
         * <p>
         * <b>与 viewUv30d 的分工</b>：uv 只数"有多少人来过"（人数），本字段进一步数
         * "其中有多少人来了不止一天"（人数 × 时间分布）——10 人各来 1 天与 5 人各来 2 天
         * 在 uv 上完全相同，在本字段上是 0 与 5 的区别。它是兴趣强度的度量，
         * 不是流量规模的度量。
         * <p>
         * 抗刷性来自天然约束（不依赖频控）：单日刷 1000 次 PV 仍只产生 1 个 view_date，
         * 脚本无法把用户刷成"复访用户"，伪造必须跨天持续操作。
         * <p>
         * <b>展示字段，不参与热度公式</b>（同 viewCount30d / favoriteCount / postCount
         * 既有范式——"下发仅供展示、不计入公式"）：进排序需在 JPQL 的 HEAT_BEHAVIOR 内
         * 表达"按用户分组后计数"，而 JPQL 无 FROM 派生表能力，须把全部列表主查询
         * native 化重写（收益/风险比不成立，论证见 VenueRepository.HEAT_SCORE 的
         * 2026-09-02 双算优化评估结论）。前端作"真实兴趣"说明性指标展示。
         */
        long repeatVisitorCount30d,
         ...
         *  "热度 76 = 近30天人气：浏览贡献 42（近30天100人 + 近7天20人）· 新增收藏人数 5×8 · …"） */
        String formulaText,
        /** 热度规则详情（完整条目化规则，前端「查看完整计算规则」点击展开——含浏览
         *  人数口径换算（近30天人数 / 近7天人数 / 匿名对数项）、各维度权重、满意度中性
         *  偏移与负向反馈规则） */
