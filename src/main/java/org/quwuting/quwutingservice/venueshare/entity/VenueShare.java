package org.quwuting.quwutingservice.venueshare.entity;
...
/**
 * 场所分享事件日志（只追加，不修改不删除——分析型数据）。
 * <p>
 * 一张表承载两类事件（eventType 区分）：
 * <ul>
 *   <li>SHARE — 分享动作：actor=分享者（user_id），channel=发起渠道（BUTTON/MENU/TIMELINE）</li>
 *   <li>OPEN — 被分享者打开详情页：actor=打开者（user_id），share_from=原分享者（归因）</li>
 * </ul>
 * 无唯一约束（事件日志语义：每次分享 / 打开都是一条独立事件，天然可 append）。
 * 匿名用户 userId 为 null，仅参与 IP 频控，不参与身份归因。
 * <p>
 * 边界：分享维度不在热度公式闭集内（公式由产品定义，见 AGENTS.md「场所热度」章节），
