...
public record DancerDetailResponse(
         ...
         * 首 2 尾 2 字符、中间 4 星；仅图片联系方式 = null，前端占位「图片联系方式」）。
         * 仅普通用户（非本人/管理员）且满足上述判定时下发；其余场景恒 null。
         * 2026-08-29 免费直显（用户拍板）：无门槛（contactCost=0）舞伴的真实值已随
         * contact/contactImageUrl 直接下发（本字段 = 冗余预览，前端直显不依赖）；
         * 有门槛舞伴真实值不下发，本字段 = 打码预览（点击揭示经 unlock 扣费查询）。
         */
         ...
         * 管理员恒返回（dancer-edit 编辑回显 + 管理者天然可见）。
         * <b>2026-08-29 免费直显例外（用户拍板）</b>：未配置服务范围（免邀约单）
         * + 非邀约中转 + 无积分门槛（contactCost=0）的舞伴 = 免费展示，真实值
         * 直接随详情下发（前端加载即展示，零点击零请求——防泄漏红线对此场景
         * 放宽，用户知情）。
         */
        String contact,
        /**
         ...
         * 本人/管理员恒返回。<b>2026-08-29 免费直显例外</b>：判定同 contact
         * （无服务范围 + 非中转 + 无门槛 → 直接下发真实图片 URL）。
         */
