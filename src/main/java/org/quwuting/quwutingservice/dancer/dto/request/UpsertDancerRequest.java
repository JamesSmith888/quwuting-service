...
public record UpsertDancerRequest(
        ...
        /**
         * 邀约中转开关（2026-08-26 新增，null = 关闭；docs/agents/22）：
         * 开启后该舞伴的联系方式获取改为「管理员中转 + 舞伴批准」——客人填邀约单
         * 后不立即拿微信（unlock 返回 PENDING），邀约进管理端「邀约工作台」，
         * 管理员微信转发给舞伴，舞伴回「给/不给」，管理员一键发放/拒绝。
         * 面向在意「把关权」（过滤口嗨）的舞伴。per-dancer 开关，存量零回归。
         */
        Boolean contactRelay,

        /**
         * 24h 无回复自动降级策略（2026-08-26 新增，null = 开启；仅 contactRelay
         * 有意义）：true = 24h 无回复自动发放联系方式（平台兜底不卡单，默认）；
         * false = 告知客人「暂未回复」（EXPIRED）。高流量舞伴建议 false——
         * 把关权被架空是她最在意的点（宁可客人流失，不让平台代发微信），配合
         * 12h 管理员微信催办，降级成为「催过无回应」的兜底。
         */
        Boolean autoRelease,
