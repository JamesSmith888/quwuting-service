...
| [`35-dashboard-stats.md`](docs/agents/35-dashboard-stats.md) | **运营数据看板（2026-09-06 P0，admin-web Dashboard + `GET /admin/users/daily-stats`）**：真实盘子口径三线趋势（注册=登录建号剔 ADMIN/test 号 / 打开=登录自动打卡 / 真实互动=当日 12 行为表去重）+ **打卡型噪音识别**（注册后从未真实互动，9-3 起占比 60%+ 疑似微信审核/巡检）；**MySQL 8 方言 mega-query**（UserDailyStatsRepository，WITH RECURSIVE 补零 + 噪音 NOT EXISTS；PG 环境勿执行）；admin-web 新增 Dashboard 页（echarts 5.5 按需注册，登录默认落地页）；噪音占比柱 ≥60% 红提示对照提审日期；P1/P2=漏斗/游客转化/分享归因/留存 cohort，admin-web 用户管理页待接 | 动运营统计口径 / Dashboard / 审核流量分析时 |
