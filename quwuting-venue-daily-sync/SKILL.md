...
- **多数据源交叉（三源制，2026-09-08 实证）**：同日可能收到 A=市井慢时光图、B=舞厅百事通
  ...
  自带别名括号归位。⚠️ **多源 = 写库门槛（2026-09-09 用户拍板）**：交叉确认结果直接决定
  能否自动写库——两源皆点名 → 确定数据（自动反转/建档 + 直发公告）；仅一源点名 → 列
  「管理员手动核实」清单不写库（含高置信反转与单源新店候选）。交叉确认实现：同一平台
  venueId 被两源各自命中，或双 UNMATCHED 同城同名（归一化/包含）互认。
...
| # | 分类 | 内容 | 列（≤5） | 确认交互 |
|---|---|---|---|---|
| ① | **可直接更新**（多源确认） | **两源以上点名** + EXACT/ALIAS 命中 + 舞讯 OPEN + 平台 CEASED/SUSPENDED（would_reverse） | 舞讯店名 \| 城市 \| 平台门店 \| 平台当前状态 \| 置信度 | 多源确认数据**直接执行**（2026-09-09 起无需等「执行」回复），写库后直发公告 |
| ② | **需用户确认**（单源/低置信） | **单源点名 + would_reverse（任意置信度）**；或多源确认但 CONTAINED 低置信 | 舞讯店名 \| 城市 \| 候选门店 \| 匹配依据 \| 来源与平台当前状态 | **列「管理员手动核实」清单**，用户逐条放行后才 forceReversal=true |
| ③ | **平台未维护**（新店候选） | **多源确认** + UNMATCHED + keyword 交叉验证同城确无同名 | 舞讯店名 \| 城市 \| note/地址 \| keyword 验证 \| 建议动作 | 多源确认新店**直接 batch-create**；单源新店先进人工核实清单，用户回复「录入」才录 |
...
> 🚨 **多来源确认门（2026-09-09 用户拍板，写库最高门槛）**：写库动作只允许「多来源
> 确认」的数据——同一家门店**两个及以上独立舞讯源同日均点名营业中**才算确定数据，
> 可直接写库（反转/建档），写库成功后**直接 create+publish 公告，无需再询问**；
> **单源点名（A 源提示开门、B 源未提示）一律不写库**——无论置信度高低，全部列入
> 「管理员手动核实」清单交用户决策（2026-09-08 #1114 单源反转回退事故的系统性收口）。
> 低置信（CONTAINED）即使多源命中也只证明「店在营业」，平台映射本身未定，仍进人工
> 清单。两源报告日期可相差一天（如 A 源昨日出稿、B 源今日出稿），汇报中注明各源
> 实际报告日期即可；守卫条目（uncertain/dict 不建库）优先级高于多源确认。

分类主键 = **多源确认 × would_reverse × 置信度**：
- 多源确认 + 高置信（EXACT/ALIAS）+ 反转 → 表①（直接执行，不再等「执行」回复）；
- 单源点名 + 反转（任意置信度）、或多源确认 + 低置信（CONTAINED）+ 反转 → 表②（人工核实）；
- 多源确认 + UNMATCHED → 表③（直接建档）；单源 + UNMATCHED → 人工核实清单；
- 其余（平台已 OPEN、平台未覆盖城市、守卫条目）→ 表④，避免噪音。
...
**分级策略（2026-09-07 用户明确；2026-09-09 多来源确认门 + 公告分开制修订）**：
- **公告已发后再补充门店（2026-09-09 用户拍板「分开」，覆盖 09-07「更新非重发」合并
  做法）**：**新发一条独立公告**（create+publish，标题如「{M}月{D}日新增门店」），
  **不要**为合并内容而改写已发布公告——恢复营业类与新增门店类各自成条；原地 update
  仅用于修正原公告自身错误/口径（如 09-08 #21 实况更正场景）；「一天一条完整公告」
  口径随之作废，同日可并存多条 DATA_UPDATE。
- **表①「多源确认数据」写库成功后：无需询问，直接按固定模板 create+publish 公告**
  （发布前仍先 `GET /admin/announcements` 核对今日是否已有 DATA_UPDATE，仅当今日已发
  且信息不全才 offline 旧条后发完整版）。⚠️ 多源确认但平台已 OPEN（当日零写库）时
  **不发公告**——无新增/恢复内容，发了是噪音，汇报中说明「确定数据均已同步」即可。
- **表②人工核实放行 / 单源新店录入 / 低置信或混合批次**等不确定数据写库后：**仍须
