...
> 守卫条目优先级高于本门。⚠️ 本门只约束表①/表⑤；**表③ 新店从不自动建档（红线 4，2026-09-11）**。
...
**分级策略（09-07 分级 → 09-09 公告分开制 → 09-10 全源一致口径 → 09-11 新店放行制）**：
- **表①（开门）全源一致数据写库成功后：无需询问，直接按固定模板 create+publish**；
...
**📋 固定模板**（category=DATA_UPDATE，pinned=true；**标题按分组选，09-14 用户纠正**：仅【新增门店】→ `{M}月{D}日新增门店`（先例 id=37/26）；含【恢复营业】→ `{M}月{D}日舞讯更新`——标题必须表达内容主体，勿一律套「舞讯更新」）：
