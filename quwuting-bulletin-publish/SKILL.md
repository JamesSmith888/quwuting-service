...
- ⚠️ **快讯没有标题字段**（2026-09-11 五稿删除，47 号 §7.7）：请求体只写 `content`，
  别再造标题（带了也会被忽略）。分享卡片标题由服务端从正文派生（前 30 字摘要）。
...
- **信息流提示**：用户不点开也能看到全文 ⇒ **正文第一句就要承担"一眼看懂"**（本域无标题可依赖），
  别把关键事实（哪家店 / 何时 / 开还是关）只写在正文末尾或图里。
- **用户可表态**：每条快讯允许用户选一个表情（平台固定字典 = 共享 emoji 目录全量 100 枚，
  无自由文本）——发布后
...
curl -s -X POST "$BASE_URL/admin/bulletins/create" \
  ...
  -d '{"content":"...","city":"重庆","offlineAt":"2026-09-12T14:00:00"}'
# → data.id；② 再发布（缺省立即）
